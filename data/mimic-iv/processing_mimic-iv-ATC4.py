...
def med_process(med_file):
    ...
    # 拼接成一个完整的 DataFrame
    med_pd = pd.concat(chunks, ignore_index=True)
    med_pd.drop(columns=['pharmacy_id','poe_id','order_provider_id','stoptime','drug_type', 'formulary_drug_cd','gsn','prod_strength',
...
def ndc2atc4(med_pd):
    ...
    med_pd['ndc'] = med_pd['ndc'].map(lambda x: x[:5]) # 紫薇修改 从5改为4
    med_pd = med_pd.drop_duplicates()    
    med_pd = med_pd.reset_index(drop=True)
    # print(med_pd)
    return med_pd
...
def create_str_token_mapping(df):
    ...
    dill.dump(obj={'diag_voc':diag_voc, 'med_voc':med_voc ,'pro_voc':pro_voc}, file=open('ATC4_voc_final_4.pkl','wb'))
...
def create_patient_record(df, diag_voc, med_voc, pro_voc):
    ...
    dill.dump(obj=records, file=open('ATC4_records_final_4.pkl', 'wb'))
    return records
...
def get_ddi_matrix(records, med_voc, ddi_file):
    ...
    dill.dump(ehr_adj, open('ATC4_ehr_adj_final_4.pkl', 'wb'))
    ...
    dill.dump(ddi_adj, open('ATC4_ddi_A_final_4.pkl', 'wb'))
...
if __name__ == '__main__':
    ...
    med_file = "./input/prescriptions.csv" # '/srv/local/data/physionet.org/files/mimiciii/1.4/PRESCRIPTIONS.csv'
    diag_file = "./input/diagnoses_icd.csv"  # '/srv/local/data/physionet.org/files/mimiciii/1.4/DIAGNOSES_ICD.csv'
    procedure_file = "./input/procedures_icd.csv" # '/srv/local/data/physionet.org/files/mimiciii/1.4/PROCEDURES_ICD.csv'
    ...
    med_structure_file = "./input/atc3toSMILES_4.pkl"
    ...
    statistics(data)
    data.to_pickle('ATC4_data_final_4.pkl')
    ...
    ddi_rate = ddi_rate_score(records, "ATC4_ddi_A_final_4.pkl")
